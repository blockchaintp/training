...
First - we need to get into a new shell inside the `xo` container.

Open a new terminal window (leaving your xo tp running) - and type the following:
...
Notice how we now have notice that there were invalid transactions in the batch.
...
> NOTE: if these commands fails because the keys already exist - you can use the `--force` flag for the `keygen` command

## Playing with different keys

Now we have some keys - lets try playing a full game with them.

First - get back to the `xo` container or if you don't have it, type `exit` in the `shell` container and then type:
