...
First - exit the xo-tp container:
...
First - lets get back into the right container:

```bash
docker exec -it sawtooth-xo-tp bash
```

And export the URL for the REST api:
...
Get yourself into the `shell` container by first getting back to the Docker host:

```bash
exit
```

Then - get into the shell container:
...
Then let's get back to the `xo-tp` container:

```bash
docker exec -it sawtooth-xo-tp bash
```

Then, export the URL for the REST api:
