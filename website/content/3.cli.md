...
So - we have one block on the chain!  Let's dig into this block and see what it contains.  We are expecting it contain a transaction that updated the `sawtooth.settings.vote.authorized_keys` setting.
